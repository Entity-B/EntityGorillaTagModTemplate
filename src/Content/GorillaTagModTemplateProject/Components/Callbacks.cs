...
namespace GorillaTagModTemplateProject.Components
{
    public class Callbacks : MonoBehaviourPunCallbacks
    {
        ...
        {
            if (FriendBackendController.Instance.FriendsList.FirstOrDefault(f => f.Presence.FriendLinkId == newPlayer.UserId) != null) FriendJoined?.Invoke(newPlayer);
            PlayerJoined?.Invoke(newPlayer);
        }
        public override void OnPlayerLeftRoom(Player otherPlayer)
        {
            if (FriendBackendController.Instance.FriendsList.FirstOrDefault(f => f.Presence.FriendLinkId == otherPlayer.UserId) != null) FriendLeft?.Invoke(otherPlayer);
